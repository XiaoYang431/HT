...
;// <e> Program Option Byte
;// <i> Program Option Byte settings into Flash (Must enable for write any one of the following settings into Option Byte Flash).
;// <i> -- Page Erase/Program Protection (OB_PPn)
;// <i> -- Security Protection (OB_CP)
;// <i> -- Watchdog Timer Auto Enable (OB_WDT)
;// <i> -- Bootloader Waiting Time.
FLASH_OPT     EQU     0
;// </e>
...
;// <e0> Bootloader Waiting Time (0x1FF0004C)
;//  <i> Setting bootloader waiting time by write timeout value to 0x1FF0004C.
;//  <i> 0xFF: Disable waiting time.
;//  <i> 0x00: Disable Bootloader.
;//  <i> 0x01~0xFE: bootloader waiting time 1550ms ~ 14200ms steps 50ms.
;//   <o1> Bootloader Waiting Time (ms, 0 = disable bootloader) <0-14200:50>
;//    <i> Bootloader waiting time based on ms. Range 1550 ~ 14200 ms, Steps: 50)
BOOT_OPT            EQU     0
BOOTOPTV            EQU     1550
;//   </e>

;// <e> Watchdog Timer Auto Enable (0x1FF003F0 or 0x1FF0002C)
;// <i> WDT will be enabled immediately when the MCU power on reset or system reset occurs. The WDT then can be disabled by firmware.
;// <i> The default timeout period of WDT is 8 second (Clock source = 32 kHz LSI, Prescaler = 1/64, 12-bit counter).
;// <i> !!! Notice !!!
;// <i>   The device below does not support Watchdog Timer Enable by Flash Option Bytes.
...
;// <e0> Writer Checksum (0x1FF00030 ~ 0x1FF0003B)
;//   <o1> Writer Checksum: Code (0x1FF00030)
;//   <o2> Writer Checksum: Data (0x1FF00034)
;//   <o3> Writer Checksum: Code + Option (0x1FF00038)
WRITERCK_OPT        EQU     0
...
OPT_CK              EQU     (nOB_PP0 + nOB_PP1 + nOB_PP2 + nOB_PP3 + nOB_CP)
    ...
    IF BOOT_OPT = 1
      IF BOOTOPTV = 0
OPT_BOOT_WAIT       EQU     0xFFFFFF00
      ELIF 1550 <= BOOTOPTV && BOOTOPTV <= 14200
OPT_BOOT_WAIT       EQU     0xFFFFFF00:OR:(BOOTOPTV - 1500)/50
      ELSE
        The_bootloader_waiting_time_is_not_available(0_or_1550_~_14200).
      ENDIF
    ELSE
    ...
    IF WDT_OPT = 1
      IF (USE_HT32_CHIP=HT32F1653_54_55_56) || \
         ...
         (USE_HT32_CHIP=HT32F52243_53)
          The_selected_MCU_does_not_support_Watchdog_Timer_Auto_Enable_(WDT_OPT=1).
      ENDIF
    ENDIF
...
OPTCK_CODEOP        EQU     0xFFFFFFFF
    ...
    IF FLASH_OPT = 1
        ...
        DCD   OB_WDT        ; 0x1FF0002C
        DCD   OPTCK_CODE    ; 0x1FF00030
        DCD   OPTCK_DATA    ; 0x1FF00034
        DCD   OPTCK_CODEOP  ; 0x1FF00038
        DCD   OPT_RES       ; 0x1FF0003C
        DCD   OPT_RES       ; 0x1FF00040
        DCD   OPT_RES       ; 0x1FF00044
        DCD   OPT_RES       ; 0x1FF00048
        DCD   OPT_BOOT_WAIT ; 0x1FF0004C
...
;// <e> Program User's Flash Data (userdata.dat) (0x1FF00050 ~ 0x1FF003EF/0x1FF002FF)
