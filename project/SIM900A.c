...
// 用短信发送函数
uint8_t SIM900A_SendSMS(const char *phone_num, const char *message) {
    uint8_t ack;
    char cmd[64];
    ...
    ack = sim900a_send_cmd((uint8_t *)cmd, (uint8_t *)">", 100);
    if(ack) return 1;
    
    // 3. 发送消息内容
    ack = sim900a_send_cmd((uint8_t *)message, (uint8_t *)">", 300);
    if(ack) return 1;
    
    // 4. 发送结束符
    return sim900a_send_end(0x1A, (uint8_t *)"OK", 800);
}
...
/**
 * @brief 拨打电话
 * @param phone_num 目标电话号码(ASCII字符串)
 * @return uint8_t 
 *   - 0: 拨号成功
 *   - 1: 拨号失败
 */
uint8_t SIM900A_MakeCall(const char *phone_num)
{
    uint8_t ack;
    char cmd[32];
    ...
    // 2. 发送指令并等待"OK"响应
    ack = sim900a_send_cmd((uint8_t *)cmd, (uint8_t *)"OK", 100);
    
    // 3. 可选：检测通话连接状态（需要额外AT指令）
    // ack |= sim900a_send_cmd((uint8_t *)"AT+CLCC\r\n", (uint8_t *)"+CLCC:", 200);
 ...
 *   - 1: 挂断失败
 */
uint8_t SIM900A_HangUp(void)
{
    // 发送ATH指令（挂断电话）
    return sim900a_send_cmd((uint8_t *)"ATH\r\n", (uint8_t *)"OK", 50);
}
