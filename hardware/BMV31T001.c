...
void BMV31T001_Init()
{
	...
	BMV31T001_delay_ms(1000);//There's a delay_ms here to get the BMV31T001 ready
    ...
    if(0xff != data)//need data
    {
        ...
        for (i = 0; i < 8; i ++)
        {
            if (1 == (cmd & temp))
            {
                    // out bit high
                    BMV31T001_HIGH(DATA);
                    BMV31T001_delay_us(1200);
                    BMV31T001_LOW(DATA);
                    BMV31T001_delay_us(400);
            }
            else
            {
                    BMV31T001_HIGH(DATA);
...
static bool  programEntry(uint16_t mode)
{
    ...
    do{
        ...
        BMV31T001_LOW(ICPCK);
        BMV31T001_delay_us(160);//tready:150us~

        /*MATCH*/

        BMV31T001_HIGH(ICPCK);

        BMV31T001_delay_us(84);
...
static void programDataOut1(void)
{
    ...
    BMV31T001_LOW(ICPCK);
    BMV31T001_delay_us(2); //tckl:1~15us
...
static void programAddrOut1(void)
{
    ...
    BMV31T001_LOW(ICPCK);
    BMV31T001_delay_us(1); //tckl:1~15us
  
    BMV31T001_HIGH(ICPCK);
    BMV31T001_delay_us(4); 
}
...
static void sendData(uint16_t data)
{
    ...
    BMV31T001_HIGH(ICPCK);
    BMV31T001_delay_us(2000);

    BMV31T001_LOW(ICPCK);
    BMV31T001_delay_us(1);

    BMV31T001_HIGH(ICPCK);
    BMV31T001_delay_us(5);
}
...
void  BMV31T001_delay_us(u32 nus)
{
//	u32 couter;
//	couter = Get_Sys_us();
...
void  BMV31T001_delay_ms(u32 nms)
{
	u32 i;	
  for(i=0;i<nms;i++)
	{
		BMV31T001_delay_us(1000);
	}
}
u8 rand_0_1(void)
{
