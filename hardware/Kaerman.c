...
float yaw_raw= 0;   		 		//航线角yaw原始数据
float yaw_kalman= 0;  	 		//yaw滤波后数据
float pitch_raw = 0;  		 	//俯仰角pitch原始数据
float pitch_kalman= 0;  	 	//pitch滤波后数据
...
static float dt      = 0.005;		//采样周期即计算任务周5ms
...
void Angle_Cal(void)
{
	...
	//获取温度
  temp = BMS56M605_readRawTemperature();    
	//获取加速度传感器数据			
  BMS56M605_readRawAcceleration(&ax, &ay, &az);
  //获得陀螺仪数据
	BMS56M605_readRawGyroscope(&gx, &gy, &gz);
	accel_x = ax;//x轴加速度值暂存
	accel_y = ay;//y轴加速度值暂存
	accel_z = az;//z轴加速度值暂存
	gyro_x  = gx;//x轴陀螺仪值暂存
