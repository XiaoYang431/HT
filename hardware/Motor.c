...
uint16_t PID_Turn(uint8_t gyro_Z)
{uint8_t Turn_Kd = 15;
	uint8_t Turn_Kp = 15;
	uint16_t PWM_out;
PWM_out = Turn_Kd * gyro_Z ;
}
...
TM_TimeBaseInitTypeDef TimeBaseIniture;         //结构体
  ...
  CKCU_PeripClockConfig_TypeDef CKCUClock = {{0}};
   ...
   CKCUClock.Bit.PA         = 1;       //GPIOA时钟打开
   CKCUClock.Bit.AFIO       = 1;       //复用时钟
   CKCUClock.Bit.PWM0      = 1;       //开启马达定时器
   CKCU_PeripClockConfig(CKCUClock, ENABLE);      //时钟使能
   AFIO_GPxConfig(HTCFG_CAP_GPIO_ID_decode3A, HTCFG_CAP_AFIO_PIN_decode3A, HTCFG_CAP_AFIO_FUN_decode3A);  //开启复用功能
   ...
   CKCUClock.Bit.PWM0      = 1;       //开启马达定时器
   CKCU_PeripClockConfig(CKCUClock, ENABLE);      //时钟使能
   AFIO_GPxConfig(HTCFG_CAP_GPIO_ID_decode3B, HTCFG_CAP_AFIO_PIN_decode3B, HTCFG_CAP_AFIO_FUN_decode3B);  //开启复用功能
		
	GPIO_InputConfig(HT_GPIOA, GPIO_PIN_14, ENABLE);       //使能输入功能
    GPIO_InputConfig(HT_GPIOA, GPIO_PIN_10, ENABLE);       //使能输入功能	 
	 	 
	 TM_DeInit(HTCFG_CAP_PORT_decode3A);    //恢复默认设置
	 TM_TimeBaseStructInit(&TimeBaseIniture);
 ...
 //对定时器时基以及计数方式初始化
	 ...
	 TimeBaseIniture.PSCReloadTime=TM_PSC_RLD_IMMEDIATE;        //立即重装载预分频寄存器
	 TimeBaseIniture.RepetitionCounter=0;                         //定时器重复寄存器 
	 TM_TimeBaseInit(HTCFG_CAP_PORT_decode3A, &TimeBaseIniture);
...
//
   ...
   TM_DecoderConfig(HTCFG_CAP_PORT_decode3A, TM_SMSEL_DECODER3,                   //模式3双边沿计数
                        TM_CHP_NONINVERTED, TM_CHP_NONINVERTED);   //编码器模式的设置
	
   TM_ChExternalClockConfig(HTCFG_CAP_PORT_decode3A, TM_TRSEL_TI0BED, TM_CHP_INVERTED, 6);//比较滤波器
	//TM_IntConfig(HTCFG_CAP_PORT_decode1A, TM_INT_CH1CC | TM_INT_UEV, ENABLE);//禁用中断

   HTCFG_CAP_PORT_decode3A->CNTR=0;      //计数器初始值
   TM_Cmd(HTCFG_CAP_PORT_decode3A,ENABLE);  //使能计数器 
}
