/**************************************************************************//**
 * @file    library/Device/Holtek/HT32F5xxxx/Source/system_ht32f5xxxx_11.c
...
/*--------------------- Clock Configuration ----------------------------------
//
//    <e1> Enable High Speed External Crystal Oscillator (HSE)
//          <i> Default HSE = DISABLE
//        <o2>  HSE Output Clock Divider
//                <0=> 1
//                <1=> 2
//                <2=> 4
//                <3=> 8
//                <i> Default HSE output clock divider = 1
//    </e>
//
//    <q4> Enable Low Speed External Crystal Oscillator (LSE)
...
#define HSE_CLK_DIV       (1)     /*!< 0: DIV1,     1: DIV2,   2: DIV4,   3: DIV8                           */
 ...
 */
  ...
  #error "CK_HSE clock issue: must be 16 MHz!"
#endif
...
/**
 * @brief CK_HSE definition
 */
#define __CK_HSE    (HSE_VALUE >> HSE_CLK_DIV)                  /*!< Get CK_HSE frequency                   */

/**
 * @brief CK_PLL definition
 */
#if (PLL_ENABLE == 1)
  /* Get CK_PLL_IN frequency                                                                                */
  #if (PLL_CLK_SRC == 1)
    #if (HSI_ENABLE == 0)
      #error "CK_PLL clock source issue: HSI has not been enabled"
    #else
      #define __CK_PLL_SRC  (HSI_VALUE >> PLL_CLK_SRC_DIV)      /*!< Select HSI as PLL source               */
    #endif
  #else
    #if (HSE_ENABLE == 0)
      #error "CK_PLL clock source issue: HSE has not been enabled!"
    #else
      #define __CK_PLL_SRC  (__CK_HSE >> PLL_CLK_SRC_DIV)      /*!< Select HSE as PLL source               */
    #endif
  #endif

  #define PLL_SRC_MIN  4000000UL
  #define PLL_SRC_MAX 16000000UL
  #define VCO_MIN 24000000UL
  #define VCO_MAX 60000000UL
  #define PLL_MIN  4000000UL
  #define PLL_MAX 60000000UL

  /* Check CK_PLL_IN frequency                                                                              */
  #if ((__CK_PLL_SRC < PLL_SRC_MIN) || (__CK_PLL_SRC > PLL_SRC_MAX))
    #error "CK_PLL_SRC clock issue: must be in the range!"
  #endif

  /* Get CK_VCO frequency                                                                                   */
  #define __CK_VCO    (__CK_PLL_SRC * PLL_NF2_DIV)              /*!< Get CK_VCO frequency                   */
...
#elif (HCLK_SRC == 2)
  #if (HSE_ENABLE == 1)
    #define __CK_SYS    __CK_HSE              /*!< Select HSE as CK_SYS source                              */
  #else
...
void SystemInit(void)
{
  ...
  /* HSE initiation                                                                                         */
#if (HSE_ENABLE == 1)
  HT_CKCU->GCFGR = (HT_CKCU->GCFGR & ~(3UL << 16)) | (HSE_CLK_DIV << 16); /* set HSE divider                */
...
void SystemCoreClockUpdate(void)
{
  ...
  u32 HseClockDiv = (HT_CKCU->GCFGR >> 16) & 3UL;
  ...
  if (SystemCoreClockSrc == 1)
  {
    ...
    {
      SystemCoreClock = ((((HSE_VALUE >> HseClockDiv) >> PllSourceClockDiv) * PllFeedbackClockDiv) >> PllOutputClockDiv) >> SystemCoreClockDiv;
    }
  }
  else if (SystemCoreClockSrc == 2)
  {
    SystemCoreClock = (HSE_VALUE >> HseClockDiv) >> SystemCoreClockDiv;
  }
